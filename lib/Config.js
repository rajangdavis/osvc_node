...
const _headersHash = (obj, patchRequest = false) => {
	...
	headersHash = _suppressionCheck(client,headersHash);
...
const _suppressionCheck = (client, headersHash) => {

	if(client.suppress_rules === true){
		headersHash['OSvC-CREST-Suppress-Rules'] = true;
	}
	if(client.suppress_events === true){
		headersHash['OSvC-CREST-Suppress-ExternalEvents'] = true;
	}
	if(client.suppress_all === true){
